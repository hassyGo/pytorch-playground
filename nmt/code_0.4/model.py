...
class WordPredictor(nn.Module):
    ...
    def __init__(self, inputDim, outputDim, ignoreIndex = -1):
        ...
        self.loss = nn.CrossEntropyLoss(reduction = 'sum', ignore_index = ignoreIndex)
...
class EncDec(nn.Module):
    ...
    def forward(self, inputTarget, lengthsTarget, lengthsSource, hidden0Target, sourceH, target = None):
        ...
        prevFinalHidden = inputTarget.new(batchSize, 1, self.targetEmbedDim).zero_()
        ...
        for i in range(maxLen):
            ...
            attentionScores = attentionScores_.new(attentionScores_.size()).fill_(-1024.0)
    ...
    def greedyTrans(self, bosIndex, eosIndex, lengthsSource, targetEmbedding, sourceH, hidden0Target, device, maxGenLen = 100):
        ...
        prevFinalHidden = sourceH.new(batchSize, 1, self.targetEmbedDim).zero_()
    ...
    def beamSearch(self, bosIndex, eosIndex, lengthsSource, targetEmbedding, sourceH, hidden0Target, device, beamSize = 1, penalty = 0.75, maxGenLen = 100):
        ...
        sourceHtrans_ = sourceHtrans.new(beamSize, sourceHtrans.size(1), sourceHtrans.size(2))
        ...
        for dataIndex in range(batchSize):
            ...
            while i < maxGenLen and not cand[0].fin:
                ...
                updatedCand = []
                updatedPrevFinalHidden = prevFinalHidden.new(prevFinalHidden.size()).zero_()
                ...
                for j in range(beamSize):
                    ...
                    updatedCand.append(DecCand(score, False, cand[row].sentence+[], cand[row].attenIndex+[attnIndex[row, 0].item()]))
